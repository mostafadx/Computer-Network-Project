...
def attacker(_ip,_port):
    ...
    dest_ip = hexlify(inet_aton(_ip))
    dest_ip = str(dest_ip)[2:-1]
    src_port = "%04x"%int(Lines[3])
    dest_port = "%04x"%int(_port)
    seq_num = "17 49 30 d1"
    ack = "00 00 00 00"
    h_len = "50 02"
    w_size = "72 10"
...
ip_addr=input('What is the target IP address? ')
port_range=input('Which ports do you want to scan? ')
port_range = port_range.split('-')
start = int(port_range[0])
finish = int(port_range[1])
fd = open('info.txt')
Lines = fd.readlines()
for i in range(start,finish):
    attacker(ip_addr,i)
    print("Send TCP SYN packet to port {}".format(i))
